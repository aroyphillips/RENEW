...
def partition_arrays(bserials: list, num_chains: int, num_boards: int, partitions: list) -> dict:
    """
    Takes list of all serials and partition types and return dictionary of transmitting and receiving arrays.
    :param bserials: list of Iris Serials available
    :param partitions:
    :return: structure: dictionary keys are partition type, values are dicionaries containing lists of tx and rx
    """
    num_srs = len(bserials)
    is_symmetric = ()

    partition_dict = {}

    for partition in partitions:
        print("This time:", partition)
        tx_boards =[]
        rx_boards =[]
        if partition == "NS":
            halfpt = num_srs//2
            tx_boards = bserials[halfpt:-1]
            rx_boards = bserials[0:halfpt]

        if partition == "EW":
            for i, serial in enumerate(bserials):
                # assign first half of each chain to be tx boards
                if (i % num_boards < num_boards/2):
                    tx_boards.extend(serial)
                else:
                    rx_boards.extend(serial)

        if partition == "NWSE":
            # optimal diagonal dependent on stack shape
            pass

        if partition == "I":
            tx_boards = bserials[::2]
            rx_boards = bserials[1::2]

        if partition == "R":
            all_idx = set(range(num_srs))
            tx_idx = random.sample(range(64),32)
            rx_idx = list(all_idx - set(tx_idx))
            tx_boards = [bserials[i] for i in tx_idx]
            rx_boards = [bserials[i] for i in rx_idx]

        # assign partitions to dictionary
        partition_dict[partition] = {"tx": tx_boards, "rx": rx_boards}
...
def main():
    ...
    parser.add_option("--partition-type", type="str", dest="partition_type",
                      help="Enter a string of the following options, separated by a comma, ex: 'EW,NS,R'\n East-West: EW\n North-South: NS\n NW-SE: NWSE\n Random: R\n All: A",
                      default="A")
    parser.add_option("--board-per-chain", type="int", dest="num_boards", help="number of boards in a single chain",
                      default=8)
    parser.add_option("--num-chains", type="int", dest="num_chains", help="number of chains in the serials list",
                      default=4)
    ...
    partitions = options.partition_type.split(",")
    if "A" in partitions:
        partitions = ["EW","NS","NWSE","I", "R"]
...
if __name__ == '__main__':
    ...
    num_strs = map(str, range(64))
    bserials= list(num_strs)
    parts = partition_arrays(bserials, 4, 8, partitions = ["EW","NS","NWSE","I", "R"])
    for part_type in parts:
        print(part_type, parts[part_type])
